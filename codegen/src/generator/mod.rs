...
use crate::config::{Service, General};
...
pub(crate) fn generate(output: &mut impl Write, nodes: Vec<Node>, general: &General, service: &Service) -> Result<()> {
    writeln!(
        output,
    ...
    if !nodes.is_empty() {
        ...
        for object in nodes {
            ...
            generate_single_object(output, object, general, service)
...
fn generate_single_object(file: &mut impl Write, node: Node, general: &General, service: &Service) -> Result<()> {
    writeln!(file, "/// Proxy object for `{}`.", node.interface)?;

    writeln!(file, "#[dbus_proxy(")?;
    {
        writeln!(file, r#"interface = "{}","#, node.interface)?;
        writeln!(file, r#"gen_blocking = {},"#, general.gen_blocking)?;
